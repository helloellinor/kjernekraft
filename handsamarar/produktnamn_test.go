...
func TestNamnKjemFråFakta(t *testing.T) {
...
// null maanader binding kom han ut som «Månadskort» — namnet på eit
// produkt han ikkje er.
func TestUsynlegMedlemskapHeldNamnetSitt(t *testing.T) {
	// `t()` gjeng gjenom singletonen, som leitar etter «mål»
	...
	for _, p := range []struct{ mål, vil string }{
		{"nn", "Svart"}, {"nb", "Svart"}, {"en", "Black Card"},
	} {
		if got := MedlemskapNamn(p.mål, svart); got != p.vil {
			t.Errorf("%s: venta «%s», fekk «%s»", p.mål, p.vil, got)
		}
	}

	// Eit usynleg medlemskap utan nykel fell attende på raanamnet, so
