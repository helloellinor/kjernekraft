...
//
// Dette er prøva på ein feil som gøymde seg godt. `ParseForm` les berre
...
// ting, av di ho berre parsar når `r.Form` er nil. Kvart felt kom
...
// `FormValue`, og dermed parsa kroppen fyrst — men berre når
...
func TestSkjemaetLesestIBåeDraktene(t *testing.T) {
	...
	for _, p := range []struct {
		...
		t.Run(p.namn, func(t *testing.T) {
			...
			if s.SerieID != 10 || s.Tittel != "Yoga" || s.Lærar != "Leon" {
...
// Eit tomt plasstal tyder «rommet rår», ikkje null plassar.
