...
// SettLøyve slær eit løyve av eller på for ein brukar. Han ligg
// bak RequireAdmin i rutaren.
//
// Svaret ber lærarlista med seg attende. Eit løyve som er slegi på her
// skal vera valbar i veljarane på den same skjermen med ein gong; utan
// lista hadde ho lege der utan aa syna seg fyrr sida vart lasta på
// nytt, og då kjennest knappen som um han ikkje gjorde noko.
func (a *App) SettLøyve(w http.ResponseWriter, r *http.Request) {
	...
	løyve := r.URL.Query().Get("loyve")
	if !database.LøyveFinst(løyve) {
		http.Error(w, "Ukjend loyve", http.StatusBadRequest)
		return
	}
	på := r.URL.Query().Get("paa") == "1"
	...
	if løyve == database.LøyveAdmin && !på {
	...
	if err := a.DB.SettLøyve(brukarID, løyve, på); err != nil {
