...
// ElevTimeplan serves the Elev timeplan (schedule) page
func (a *App) ElevTimeplan(w http.ResponseWriter, r *http.Request) {
	...
	targetMonday := VikeMåndag(now, weekOffset)

	// Get events for the target week
	weekEvents, err := a.DB.GetEventsForWeek(targetMonday, int64(user.ID))
	if err != nil {
	...
	if teacherFilter != "" || classFilter != "" {
		...
		for _, event := range weekEvents {
			...
			// Yoga». Han ser på slaget no — og «reformer» er ikkje eit
	...
	if len(weekEvents) > 0 {
		...
		userSignups, err := a.DB.GetUserSignupsForEvents(int64(user.ID), eventIDs)
		if err != nil {
	...
	teachers, err := a.DB.GetDistinctTeachers()
	if err != nil {
		teachers = []string{} // Continue with empty list if error
	}

	// Kategoriar, ikkje titlar. Lista kom frå DISTINCT class_type og
	// gav tretten val — «boksing», «spinning», «crossfit» — eit studio
	// som ikkje finst. Ein filtrerer på kva slag time det er, og
	...
	// han stend her og filtrerer på rommet.
	classTypes := []string{"yoga", "pilates", "reformer", "fascia"}

	data := sidedata(r, SidaTimeplan, "Timeplan", map[string]any{
		"WeekTitle":       weekTitle,
		...
		// Pilene er lenkjor og ikkje knappar med `onclick`. Adressa er
		// alt dei treng, og ho ber sikti med seg: bladar du ei vike fram
		// medan du ser paa Kristina sine timar, skal du sjaa Kristina
		// sine timar i den vika og.
		"ForrigeVeke": adresseMed(r, "week", strconv.Itoa(weekOffset-1)),
		"NesteVeke":   adresseMed(r, "week", strconv.Itoa(weekOffset+1)),
		"User":        user,
	})
