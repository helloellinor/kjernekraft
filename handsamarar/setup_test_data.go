...
// SetupTestData creates test users and data for development
...
func (a *App) setupTestData() error {
	// Create test user
	...
	existingUser, _ := a.DB.AuthenticateUser(testUser.Email, "password123")
	if existingUser != nil {
		log.Println("Test user already exists, skipping user creation")
		return a.setupMembershipAndKlippekortData(int64(existingUser.ID))
	}

	userID, err := a.DB.CreateUser(testUser)
	if err != nil {
		return err
	}
	...
	return a.setupMembershipAndKlippekortData(userID)
}

func (a *App) setupMembershipAndKlippekortData(userID int64) error {
	// Create membership types if they don't exist
	_, err := a.DB.Conn.Exec(`INSERT OR IGNORE INTO memberships (id, name, price, commitment_months, is_student_senior, is_special_offer, description, features, active) VALUES 
	...
	_, err = a.DB.Conn.Exec(`INSERT OR IGNORE INTO klippekort_packages (id, name, category, klipp_count, price, price_per_session, description, valid_days, active, is_popular) VALUES 
	...
	existingMembership, _ := a.DB.GetUserMembership(userID)
	if existingMembership == nil {
		...
		_, err = a.DB.Conn.Exec(`INSERT INTO user_memberships (user_id, membership_id, status, start_date, renewal_date, binding_end, last_billed) VALUES (?, 1, 'active', ?, ?, ?, ?)`,
	...
	if kortTal == 0 {
		...
		_, err = a.DB.Conn.Exec(`INSERT INTO user_klippekort (user_id, package_id, total_klipp, remaining_klipp, expiry_date, purchase_date) VALUES 
			(?, 2, 10, 7, ?, ?),
			(?, 4, 5, 3, ?, ?)`,
