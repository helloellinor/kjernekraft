...
// i den same malen. På heimesida gjekk det, av di tenaren teiknar
// bolken der og skriptet fylgjer med sida. På klippekortsida vert han
// henta med `fetch` og sett inn med `innerHTML`: knappane synte seg,
// funksjonen fanst ikkje, og eit trykk gav ein `ReferenceError` i
// konsollen og ingen ting på skjermen. «Fyll på» gjorde ingen ting, og
// berre på den eine sida.
//
...
// `static/js/klippfyll.js` og fangar trykket på `document`, so han bryr
// seg ikkje um når knappen kom.
func TestKlippekortbolkenBerIkkjeSkriptetSitt(t *testing.T) {
	...
	kort := modules.NewKlippekortModule([]models.KlippekortWithDetails{{
		...
		UserKlippekort: models.UserKlippekort{
			RadID: 1, TotalKlipp: 10, RemainingKlipp: 3,
