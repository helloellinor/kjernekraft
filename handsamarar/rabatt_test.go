...
// stå — og det er den slags feil ingen oppdagar, av di ho ser ut som
// ingen ting.
func TestRabattenGjengUt(t *testing.T) {
	...
	for _, p := range []struct {
		...
		{"gjeld enno", models.User{
			StudentSenior: true, StudentSeniorGjengUt: nå.AddDate(1, 0, 0)}, true},
		{"gjekk ut i gaar", models.User{
			StudentSenior: true, StudentSeniorGjengUt: nå.AddDate(0, 0, -1)}, false},
		...
		{"honnør kjem av alderen og gjeng aldri ut", models.User{
			Birthdate: nå.AddDate(-70, 0, 0).Format("2006-01-02")}, true},
		{"for ung til honnør", models.User{
			Birthdate: nå.AddDate(-30, 0, 0).Format("2006-01-02")}, false},
	} {
...
func TestGrensaGjengVedDatoen(t *testing.T) {
	nå := config.GetInstance().GetCurrentTime()
	knapt := models.User{StudentSenior: true, StudentSeniorGjengUt: nå.Add(time.Hour)}
	nettupp := models.User{StudentSenior: true, StudentSeniorGjengUt: nå.Add(-time.Hour)}
