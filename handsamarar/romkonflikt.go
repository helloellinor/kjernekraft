...
// RoomConflict segjer um rommet alt er uppteke.
//
...
func (a *App) RoomConflict(w http.ResponseWriter, r *http.Request) {
	q := r.URL.Query()
	...
	kollisjon, err := a.DB.RoomConflict(romID, start, slutt)
	if err != nil {
	...
	if kollisjon != nil {
		...
		svar["message"] = fmt.Sprintf("%s %s–%s: %s (%s)",
			...
			veggklokka(kollisjon.Start).Format("15:04"),
			veggklokka(kollisjon.Slutt).Format("15:04"),
			kollisjon.Tittel, kollisjon.Lærar)
	}
