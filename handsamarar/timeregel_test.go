...
// Ein time på gjeven vekedag og klokkeslett, n veker fram i tidi.
func prøvetime(tittel, lærar string, start time.Time, lengdMin int) models.Event {
	return models.Event{
		Title:       tittel,
		TeacherName: lærar,
		RoomName:    "Salen",
...
func TestGrupperTimarLesReglaneUtOrRadene(t *testing.T) {
	...
	måndag := time.Date(2026, 8, 31, 18, 0, 0, 0, sona)

	var timar []models.Event
	// Aatte veker yoga med Leon — éin serie.
	for v := 0; v < 8; v++ {
		timar = append(timar, prøvetime("Yoga", "Leon", måndag.AddDate(0, 0, 7*v), 60))
	}
	// Same timen, men i fjor — skal ikkje med.
	timar = append(timar, prøvetime("Yoga", "Leon", måndag.AddDate(-1, 0, 0), 60))
	// Ein annan lærar på same timen er ein annan serie.
	timar = append(timar, prøvetime("Yoga", "Kari", måndag.AddDate(0, 0, 14).Add(-time.Hour), 60))
	...
	if seriar[0].Lærar != "Kari" || seriar[1].Lærar != "Leon" {
		t.Fatalf("venta Kari fyrst og Leon so, fekk %q og %q", seriar[0].Lærar, seriar[1].Lærar)
	}
...
func TestRegelIDenAvgjerGrupperingi(t *testing.T) {
	...
	a := prøvetime("Yoga", "Leon", måndag, 60)
	a.SerieID = 42
	b := prøvetime("Yoga", "Vikar-Kari", måndag.AddDate(0, 0, 7), 60)
	b.SerieID = 42
	...
	if seriar[0].Lærar != "Leon" {
		t.Fatalf("venta at serien les læraren or den fyrste timen, fekk %q", seriar[0].Lærar)
	}
}
...
func TestVikarenPåFyrsteTimenVertIkkjeLaerarenIRekkja(t *testing.T) {
	måndag := time.Date(2026, 9, 7, 18, 0, 0, 0, time.Local)

	var timar []models.Event
	for i := 0; i < 6; i++ {
		e := prøvetime("Yoga", "Leon", måndag.AddDate(0, 0, 7*i), 60)
		e.SerieID = 12
		if i == 0 {
	...
	seriar := GrupperTimar(timar, måndag.AddDate(0, 0, -1))
	if len(seriar) != 1 {
	...
	if r.Lærar != "Leon" {
		t.Errorf("læraren i rekkja er %q; ein vikar paa éin time av seks skal ikkje taka rekkja", r.Lærar)
	}

	// Og lista skal syna båe: rekkja er ikkje éin lærar heile vegen,
	// og det er nett det ein treng aa sjå.
...
func TestRekkjaUtanVikarBerEittNamn(t *testing.T) {
	...
	r := GrupperTimar(timar, måndag.AddDate(0, 0, -1))[0]
	if r.Lærar != "Leon" || len(r.Laerarar) != 1 {
		t.Errorf("venta berre Leon, fekk %q %v", r.Lærar, r.Laerarar)
	}
}
