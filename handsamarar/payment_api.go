...
//
// Dei er paadikta enno — Stripe er ikkje kopla på — og det er nettupp
// difor dei lyt koma frå éin stad. Briefingen på sida tel dei og
...
// PaymentMethods provides HTMX endpoint for user's payment methods
func (a *App) PaymentMethods(w http.ResponseWriter, r *http.Request) {
	user := brukaren(r)
...
// Charges provides HTMX endpoint for user's charges/billing history
func (a *App) Charges(w http.ResponseWriter, r *http.Request) {
	user := brukaren(r)
...
func (a *App) SetDefaultPaymentMethod(w http.ResponseWriter, r *http.Request) {
...
func (a *App) RemovePaymentMethod(w http.ResponseWriter, r *http.Request) {
	paymentMethodIDStr := r.FormValue("payment_method_id")
	_, err := strconv.Atoi(paymentMethodIDStr)
	if err != nil {
		http.Error(w, "Invalid payment method ID", http.StatusBadRequest)
