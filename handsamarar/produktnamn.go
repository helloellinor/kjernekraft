...
func MedlemskapNamn(lang string, m models.Membership) string {
	...
	// «Black» der, «Svart» på norsk og «Black Card» på engelsk. Difor
	// eit uppslag og ikkje ein fast streng — og eit uppslag som fell
	// attende på raanamnet, so eit nytt usynleg medlemskap syner seg
	// med ein gong og fær sitt eige ord når nokon skriv nykelen.
	// Sjå database/svartmedlem.go.
	if m.Skjult {
