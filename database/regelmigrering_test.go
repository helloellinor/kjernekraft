...
// Etterfyllinga av rule_id gjeng no i same økta som ALTER TABLE, og ei
...
func TestRegelKolonneOgEtterfylling(t *testing.T) {
	...
	var reglar, utan int
	if err := db.QueryRow("SELECT COUNT(DISTINCT rule_id), COUNT(*) FILTER (WHERE rule_id IS NULL) FROM events").
	...
	if err := db.QueryRow(`SELECT COUNT(DISTINCT rule_id), MIN(rule_id) FROM events WHERE title = 'Yoga'`).
