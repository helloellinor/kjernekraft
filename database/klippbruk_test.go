...
// Oppsettet: ein reformer-time, eit reformer-kort med klipp på, og ein
...
func klippoppsett(t *testing.T, db *Database, timetype string, klipp int) (eventID, userID int64) {
	...
	res, err := db.Conn.Exec(`
		...
		VALUES ('Time', ?, ?, ?, 4)`,
		veggtekst(nå), veggtekst(nå.Add(time.Hour)), timetype)
	if err != nil {
	...
	if _, err := db.Conn.Exec(
		...
		userID, eventID, veggtekst(nå)); err != nil {
...
// Krysset tek eit klipp. Det var heile mangelen: ein kunde gaa på
...
// Tvo trykk på den same knappen er ein ting som hender i ein kiosk.
...
func TestTvoKryssTekBerreEitKlipp(t *testing.T) {
	...
	nå := time.Now()
	for i := 0; i < 3; i++ {
		if err := db.MerkFrammote(eventID, userID, nå); err != nil {
...
// gjenom, og skuldi skal stå att so administrasjonen ser henne.
...
// Feil person kryssa av: klippet kjem attende på det kortet det vart
// teke frå.
...
// Å angra tvo gonger skal ikkje trylla fram klipp or inkje.
