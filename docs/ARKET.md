...
| `.dagmerke` | a clock in a tall case. Geometry is computed in `handlers/merkeform.go`; `kroppH` is the one number you tune, everything else derives from it. Size is one scale and nothing else. The gradient and filters live **inside each figure**, never in a shared `<defs>` — a `stop-color: var(--glans)` resolves where the gradient is written, so a shared set gives every mark the root theme's colours. The seat count is a **number**, not a pie: a fraction cannot be told apart (2 of 10 looks like 4 of 20). **Selected colours the face, not only the edge** (added 2026-08-28). A mark can be selected two ways — click the mark, or click anywhere in its row — and the two used to look different: clicking the mark left the pointer *on* it, so you got `:hover` as well and the face went teal; clicking the row left the pointer elsewhere, so you got the ring and the edge and nothing else — a teal box around an otherwise unchanged mark. One state must not have two pictures, and it must not depend on where the hand is resting. `aria-pressed` now tints `.form-flate` at `--merke` 12 % against `--flate`, one step above the 7 % hover puts there: selected reads heavier than hovered, and a mark you already picked does not fade when you sweep over it. The rule carries `:not(:disabled)` to match the hover rule's weight, and wins on file order. |
