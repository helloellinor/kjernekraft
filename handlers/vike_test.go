...
func TestVikenummeretVeksMedEittPerOffset(t *testing.T) {
	// Ein torsdag midt i aaret, og ein som ligg tett paa aarsskiftet.
	for _, start := range []time.Time{
		time.Date(2026, 8, 27, 12, 0, 0, 0, time.UTC),
		time.Date(2026, 12, 17, 12, 0, 0, 0, time.UTC),
	} {
		_, fyrre := VikeMaandag(start, 0).ISOWeek()
		vekerIAar := VikorIAaret(VikeMaandag(start, 0))

		for offset := 1; offset <= 60; offset++ {
			_, denne := VikeMaandag(start, offset).ISOWeek()
			venta := fyrre + 1
			if venta > vekerIAar {
				venta = 1
				vekerIAar = VikorIAaret(VikeMaandag(start, offset))
			}
			if denne != venta {
...
// sundag kveld skal sjaa vika han er ferdig med, ikkje den som byrjar
...
// Kvar maandag VikeMaandag gjev, skal faktisk vera ein maandag — ogso
// naar ein kryssar sommartid.
func TestVikeMaandagGjevAlltidEinMaandag(t *testing.T) {
	...
	for offset := 0; offset <= 80; offset++ {
		if d := VikeMaandag(start, offset).Weekday(); d != time.Monday {
