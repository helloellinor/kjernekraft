...
// Kva som stend i skjemaet, avgjort av tvo spursmaal som ikkje er det
...
// tilfellet ein ikkje tenkjer paa: `GrupperTimar` samlar serielause
// timar paa samantreff av tittel, lærar, rom, vekedag, klokke og lengd,
...
func TestSkjemaetFylgjerSerienOgTalet(t *testing.T) {
	...
	for _, tf := range tilfelle {
		t.Run(tf.namn, func(t *testing.T) {
			...
			// Lista er alltid i dokumentet — merket bur paa rada.
