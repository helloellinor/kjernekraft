...
func TestHelsingaSegjerDenSameKlokkaSomTimen(t *testing.T) {
	...
	fekk := HelsingNaar("nn", &models.Event{StartTime: start}, naa)
...
func TestTittelenNyttarBerreFyrenamnet(t *testing.T) {
	naa := time.Date(2026, 8, 27, 9, 0, 0, 0, time.UTC)
	fekk := HelsingTittel("nn", "Ellinor Linnea Stokke", "i morgon tidleg", naa, false)
...
// kombinasjon paa alle tri maali og les etter nettupp det.
func TestBriefingenSegjerAldriNokoRart(t *testing.T) {
	// Umsetjingane fyrst: `lastMaali` leitar etter «../locales» og lyt
	// difor lesa medan me endaa stend i pakkekatalogen. Etterpaa gjeng
	// me eit hakk upp, av di malane vert leita etter fraa rota.
	lastMaali(t)
		...
		for _, harTime := range []bool{true, false} {
			for _, iVeka := range []int{0, 1, 3} {
				for _, klipp := range []int{0, 1, 30} {
					...
					b := NyBriefing(lang, e, naa, iVeka, klipp)
					...
					// Punktum eller utropsteikn: baae sluttar ei setning.
					// «Matta kallar på deg!» er ei oppmoding, og ei
					// oppmoding endar ikkje paa punktum.
					...
					// Lekkane skal berre stande naar dei hev noko aa segja.
