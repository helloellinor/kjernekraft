...
// utan dialogar.
//
// Ein mal som ikkje parsar syner seg fyrst naar nokon opnar sida, og ein
...
func TestFlytaneTeiknarSeg(t *testing.T) {
	...
	sider := []struct {
		...
		{"pages/medlemskapet", map[string]interface{}{
			...
			"Tabs":          []Tab{{Key: "medlemskapet", Name: "Medlemskapet"}, {Key: "byt", Name: "Byt"}},
			"SwitchOptions": nil,
		}, []string{
		...
		{"pages/betaling", map[string]interface{}{
			"Lang": "nn", "Title": "t", "CSRFToken": "x",
		}, []string{`id="betalingssvar"`, `class="merknad"`, "filterCharges"}},
		// Rutenetet lyt teiknast med noko i seg. Med ei tom lista gjeng
		// `{{range .ClassRows}}` aldri inn i kroppen sin, og daa vert
		...
		//
		// Rada kjem fraa `BuildWeekRows` og ikkje fraa ein literal, so
		...
		}, []string{"navigateWeek", "applyFilters",
...
func proveRader() []ClassRow {
	naa := time.Date(2026, 8, 26, 9, 0, 0, 0, time.UTC) // ein onsdag
	maandag := VikeMaandag(naa, 0)
	// Torsdag, altso *framfyre* «no». Timen laag paa tysdag fyrr, og daa
	// var han gjengen — og ein gjengen time vert ikkje teikna lenger, so
	// rada fanst ikkje og prøva fann korkje namn eller merke.
	start := maandag.AddDate(0, 0, 3).Add(18 * time.Hour) // torsdag 18:00
	e := models.Event{
	...
	return BuildWeekRows("nn", []models.Event{e}, naa, maandag)
}
